...
mat4 orthographicProjectionMatrix; // mat4 -> vmath.h
...
void display(void)
{
	...
	mat4 modelViewProjectionMatrix = orthographicProjectionMatrix * modelViewMatrix; //order of both matrix is not associative

	// Ppush above mvp into vertex shader's mvpuniform
	glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);

	glBindVertexArray(vao);
	glDrawArrays(GL_TRIANGLES, 0, 3); //~GL_POINTS
	glBindVertexArray(0);
