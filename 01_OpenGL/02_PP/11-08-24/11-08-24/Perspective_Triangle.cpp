...
int initialize(void)
{
	...
	glShaderSource(tesselationControlShaderObject, 1, (const GLchar**)&tesselationControlSourceCode, NULL);
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(gpFile, "Tesselation Control Compilation Shader compilation Error Log : %s \n", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
	...
	glShaderSource(tesselationEvaluationShaderObject, 1, (const GLchar**)&tesselationEvaluationSourceCode, NULL);
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(gpFile, "Tesselation Evaluation Compilation Shader compilation Error Log : %s \n", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
	...
	mvpmatrix uniform = glGetUniformLocation(shaderProgramObject, "umvpmatrix");
	...
	const GLfloat point_position[] = 
				{
					...
					-0.5, 1.0,			// 2nd Quadrant
					0.5, -1.0,			// 4th Quadrant
					1.0, 1.0,			// 1st Quadrant
				};

	const GLfloat point_color[] =
				{
					1.0f, 1.0f, 1.0f,		// glColor3f(1.0f, 0.0f, 0.0f); from FFP
					1.0f, 1.0f, 1.0f,		// glColor3f(0.0f, 1.0f, 0.0f); from FFP
					1.0f, 1.0f, 1.0f		// glColor3f(0.0f, 0.0f, 1.0f); from FFP
				};
...
void display(void)
{
	...
	gluniform4fv(lineColorUniform, 1, vec4(1.0, 1.0, 0.0, 1.0));

	// Display changing no of segments in win title/ caption bar
	TCHAR str[255];
	wsprintf(str, TEXT("Srushti Umesh Moghe [No of Line Segments = %d]"), uiNumberOfLineSegments);
	SetWindowText(ghwnd, str);

	// Tell OpenGL by howmany vertices is your one patch created
