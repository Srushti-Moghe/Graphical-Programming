...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	*/
	...
	gpFile = fopen("09_Two2DShapesColoured.txt", "w");
	if (gpFile == NULL)
	{
...
void display(void)
{
	...
	glMatrixMode(GL_MODELVIEW);			// CurrentTransformationMatrix (CTM) - is the Model View Matrix
	glLoadIdentity();

	// For Triangle

	// Here Z-coord has shifted (-6) units on X-axis
	...
	// Here Z-coord has shifted (-6 + 0)i.e. -6 units on Z-axis
	...
	// Here Z-coord has shifted (-6 + 0)i.e. -6 units on Z-axis
	glTranslatef(3.0f, 0.0f, -0.0f);	// X-coord is to be moved 3 units to the right because the triangle is at -1.5 and rectangle is at +1.5
