...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	gpFile = fopen("Coloured_Two2DShapes.txt", "w");
	if (gpFile == NULL)
	{
...
void update(void)
{
	...
	rAngle = rAngle - 0.008f;		// tAngle in "degrees"

	if (rAngle <= 0.0f)
	{
		rAngle = rAngle + 360.0f;
		// rAngle = 0.0f; Also Works
	}

	tAngle = tAngle + 0.008f;		// tAngle in "degrees"

	if (tAngle >= 360.0f)
	{
		tAngle = tAngle - 360.0f;
		// tAngle = 0.0f; Also Works
	}
}
