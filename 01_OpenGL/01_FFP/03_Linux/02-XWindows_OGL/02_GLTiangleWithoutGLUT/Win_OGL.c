...
int main(void)
{
    ...
    while (bDone == False)
    {
        ...
        while (XPending(display))
        {
            ...
            XNextEvent(display, &event); // Fourteenth API
...
void uninitialize(void)
{
    ...
    if (window)
    {
        XDestroyWindow(display, window); // Sixteenth API
    }

    if (colormap)
    {
        XFreeColormap(display, colormap); // Seventeenth API
    }

    if (display)
    {
        XCloseDisplay(display); // Eighteenth API
        display = NULL;
    }
