...
HRESULT initialize(void)
{
	...
	// Create Vertex buffer for above triagle COLOR
	ZeroMemory((void *)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));

	d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3d11BufferDesc.ByteWidth = sizeof(pyramid_color);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Color buffer Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Color buffer Success\n");
		fclose(gpFile);
	}
	// Set above buffer in pipelin in "display" cuzit is dynamic
	...
	// Map our position buffer with above resource
	gpID3D11DeviceContext->Map(gpID3D11Buffer_ColorBuffer,
							   0,
							   D3D11_MAP_WRITE_DISCARD,
							   0,
							   &d3d11MappedSubresource);
	// Now copy actual data
	memcpy(d3d11MappedSubresource.pData, pyramid_color, sizeof(pyramid_color));
