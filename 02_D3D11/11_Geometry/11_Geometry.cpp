...
HRESULT initialize(void)
{
	...
	const char *geometryShaderSourceCode =
		...
		"};\n"
		"[maxvertexcount(9)]\n" // In DirectX Geometry Shader MUST NOT return anything, it should write its output into a stream.
	...
	// Create the Geometry shader from above code
	hr = gpID3D11Device->CreateGeometryShader((void *)pID3DBlob_GeometryShaderSourceCode->GetBufferPointer(),
											  pID3DBlob_GeometryShaderSourceCode->GetBufferSize(),
											  NULL,
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create Geometry Shader Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create Geometry Shader Success\n");
		fclose(gpFile);
	}

	// Set above created Geometry shader into pipeline
	gpID3D11DeviceContext->GSSetShader(gpID3D11GeometryShader,
									   NULL,
									   0);
	...
	gpID3D11DeviceContext->VSSetConstantBuffers(0,
												1,
												&gpID3D11Buffer_ConstantBuffer);
	gpID3D11DeviceContext->GSSetConstantBuffers(0,
												1,
												&gpID3D11Buffer_ConstantBuffer);
