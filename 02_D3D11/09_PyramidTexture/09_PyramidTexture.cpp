...
#pragma comment(lib, "DirectXTK.lib")	// D3D related liabriries
...
// Uncull
ID3D11RasterizerState *gpId3dRasterizerState;
...
HRESULT initialize(void)
{
	...
	const char *pixelShaderSourceCode =
		...
		"float4 main(vertex_output v_in):SV_TARGET\n"
		"{\n"
		"float4 color = myTexture2D.Sample(mySamplerState,v_in.texcoord);\n"
		"return(color);\n"
		"}\n";
	...
	// Structure Array Zero Memory
	ZeroMemory((void *)d3dInputElementDesc,
			   sizeof(D3D11_INPUT_ELEMENT_DESC) * _ARRAYSIZE(d3dInputElementDesc));
	...
	d3dInputElementDesc[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT; // Similar to glPixelStorei
	...
	hr = gpID3D11Device->CreateInputLayout(d3dInputElementDesc,
										   _ARRAYSIZE(d3dInputElementDesc), // No hardcoded array size
	...
	d3d11BufferDesc.ByteWidth = sizeof(pyramid_position);
	...
	memcpy(d3d11MappedSubresource.pData, pyramid_position, sizeof(pyramid_position));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_PositionBuffer, 0);

	// Create Vertex buffer for above triagle TEXTURE
	ZeroMemory((void *)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));

	d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	d3d11BufferDesc.ByteWidth = sizeof(pyramid_texcoord);
	d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "TexCoord buffer Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "TexCoord buffer Success\n");
		fclose(gpFile);
	}
	// Set above buffer in pipelin in "display" cuzit is dynamic
	...
	// Map our position buffer with above resource
	gpID3D11DeviceContext->Map(gpId3d11Buffer_TexCoordBuffer,
							   0,
							   D3D11_MAP_WRITE_DISCARD,
							   0,
							   &d3d11MappedSubresource);
	...
	gpID3D11Device->CreateRasterizerState(&d3dRasterizerDesc, &gpId3dRasterizerState);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create and set Rasterizer state Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create and set Rasterizer state Success\n");
		fclose(gpFile);
	}
	...
	// Load Texture
	hr = LoadD3DTexture(L"stone.bmp", &gpId3dShaderResourceView);

	// L -> Literal (ANSI to wchar_t)
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "LoadD3DTexture() Success\n");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "SamplerState() Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "SamplerState() Success\n");
		fclose(gpFile);
	}
...
HRESULT LoadD3DTexture(const wchar_t *textureFileName, ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "CreateWICTextureFromFile() Error\n");
		fclose(gpFile);
		return hr;
	}
	else
...
HRESULT resize(int width, int height)
{
	...
	// Create an empty texture according to the chnaged size call it as depth buffer/ depth texture
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Depth Buffer texture creation Failed\n");
		fclose(gpFile);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Depth Buffer texture creation Success\n");
		fclose(gpFile);
	}

	// Create D S V acc to Depth buffer structrure
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Depth Stencil View Failed\n");
		fclose(gpFile);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Depth Stencil View Success\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	// Set Texture Samplet State  in Pixel Shader
	...
	// draw the Geometry
	gpID3D11DeviceContext->Draw(12, 0); // No of Surfaces
...
{
	// Code
	pAngle = pAngle + 0.1f; // tAngle in "degrees"
	if (pAngle >= 360.0f)
	{
...
void uninitialize(void)
{
	...
	if (gpId3dRasterizerState)
	{
		gpId3dRasterizerState->Release();
		gpId3dRasterizerState = NULL;
	}
