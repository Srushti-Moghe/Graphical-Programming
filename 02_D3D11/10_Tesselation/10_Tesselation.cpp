...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case VK_UP: 
		...
		{
			uiNumberOfLineSegments = 30; //max lines 30
		}
		break;
	case VK_DOWN: 
		uiNumberOfLineSegments--;
		...
		{
			uiNumberOfLineSegments = 1; //min line initiate to 0
		}
		break;
	}
	break;
...
HRESULT initialize(void)
{
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Hull Shader Compilation Error = %s\n", (char*)pID3DBlob_Error->GetBufferPointer());
		fclose(gpFile);
		return hr;
	}
	else
	...
	// Create the Hull shader from above code
	hr = gpID3D11Device->CreateHullShader((void*)pID3DBlob_HullShaderSourceCode->GetBufferPointer(),
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create Hull Shader Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create Hull Shader Success\n");
		fclose(gpFile);
	}

	// Set above created Hull shader into pipeline
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Domain Shader Compilation Error = %s\n", (char*)pID3DBlob_Error->GetBufferPointer());
		fclose(gpFile);
		return hr;
	}
	else
	...
	// Create the Domain shader from above code
	hr = gpID3D11Device->CreateDomainShader((void*)pID3DBlob_DomainShaderSourceCode->GetBufferPointer(),
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create Domain Shader Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create Domain Shader Success\n");
		fclose(gpFile);
	}

	// Set above created Domain shader into pipeline
	...
	memcpy(d3d11MappedSubresource.pData, point_position, sizeof(point_position));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_PositionBuffer, 0);

	// Create constant buffer to send transformation like uniform data >> 1 Hull Shader
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create ConstantBuffer_Hull_Shader Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create ConstantBuffer_Hull_Shader Success\n");
		fclose(gpFile);
	}
	...
	// Create constant buffer to send transformation like uniform data >>> 2 Domain Shader
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create ConstantBuffer_Domain_Shader Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create ConstantBuffer_Domain_Shader Success\n");
		fclose(gpFile);
	}
	// Set above buffer into pipeline
	...
	// Create constant buffer to send transformation like uniform data >> 3 Pixel Shader
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create ConstantBuffer_Pixel_Shader Failed\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "Create ConstantBuffer_Pixel_Shader Success\n");
		fclose(gpFile);
	}
	// Set above buffer into pipeline
...
HRESULT resize(int width, int height)
{
	...
	if (width <= height)
	{
		perspectiveProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f,
																	   100.0f,
	...
	{
		perspectiveProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f * ((float)width / (float)height),
...
void display(void)
{
	...
	XMMATRIX worldMatrix = XMMatrixTranslation(0.f, 0.0f, 3.0f);
	...
	// Check 
	constantbuffer_hull_shader.Hull_Constant_Params = XMVectorSet(1.0, uiNumberOfLineSegments, 0.0, 0.0);
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_Hull_Shader, 0, NULL, &constantbuffer_hull_shader, 0, 0);
	// Check
	TCHAR str[255];
	wsprintf(str, TEXT("SUM D3D Window %2d"), uiNumberOfLineSegments);
	...
	constantbuffer_pixel_shader.Line_Color = XMVectorSet(1.0f, 1.0f, 0.0f, 1.0f); // Yellow Color
